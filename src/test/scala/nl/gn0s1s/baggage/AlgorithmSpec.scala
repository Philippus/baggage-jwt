...
object AlgorithmSpec extends Properties("Algorithm") {
  property("returns None for not supported algorithms") = forAll { (a: String) =>
  ...
  property("signing fails if keys is empty") = forAll { (alg: Algorithm, data: String) =>
    (alg != NoneAlgorithm) ==>
  ...
  propertyWithSeed("verifying fails if key is invalid", None) = forAll {
    (alg: Algorithm, data: String, key: Key, verificationKey: Key) =>
      (!key.value.sameElements(
        verificationKey.value
      ) && key.value.nonEmpty && verificationKey.value.nonEmpty && alg != NoneAlgorithm &&
        !alg.sign(data, key).get.sameElements(alg.sign(data, verificationKey).get)) ==>
        !alg.verify(data, verificationKey, encodeSignature(alg.sign(data, key).get).get)
  }

  property("verifying fails if signing fails") = forAll { (alg: Algorithm, data: String, key: Key) =>
