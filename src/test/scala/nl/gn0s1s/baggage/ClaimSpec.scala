...
object ClaimSpec extends Properties("Claim") {
  ...
  property("issuer claim accepts stringOrUri") = forAll { s: String =>
    !s.contains(':') ==> IssuerClaim(s).isValid
  }

  property("subject claim accepts stringOrUri") = forAll { s: String =>
    !s.contains(':') ==> SubjectClaim(s).isValid
  }

  property("audience claim accepts stringOrUri") = forAll { s: String =>
    !s.contains(':') ==> AudienceClaim(s).isValid
  }

  property("audience claim accepts several stringOrUri's") = forAll { l: List[String] =>
    l.forall(!_.contains(':')) ==> AudienceClaim(l).isValid
  }

  property("audience claim rejects non-stringOrUri's") = forAll { i: Int =>
    !AudienceClaim(i).isValid
  }

  property("audience claim rejects several non-stringOrUri's") = forAll { l: List[Int] =>
    l.nonEmpty ==> !AudienceClaim(l).isValid
  }

  property("expiration time claim accepts numbers >= 0") = forAll { l: Long =>
    (l >= 0) ==> ExpirationTimeClaim(l).isValid
  }

  property("not before claim accepts numbers >= 0") = forAll { l: Long =>
    (l >= 0) ==> NotBeforeClaim(l).isValid
  }

  property("issued at claim accepts numbers >= 0") = forAll { l: Long =>
    (l >= 0) ==> IssuedAtClaim(l).isValid
  }

  property("jwt id claim accepts strings") = forAll { s: String =>
    JwtIdClaim(s).isValid
  }

  property("public claims are always valid") = forAll { (s: String, a: AnyVal) =>
    PublicClaim(s, a).isValid
  }

  property("private claims are always valid") = forAll { (s: String, a: AnyVal) =>
  ...
  property("processor can process examples in the wild - example in rfc 7519") = {
    ...
    ClaimsProcessor
      .process(
        claims,
        Set("iss", "exp", "http://example.com/is_root"),
        Set(IssuerClaim("joe"), PrivateClaim("http://example.com/is_root", true)),
        Duration.ofSeconds(current - 1300819380 + 60)
      )
      .isSuccess
  }
  ...
  property("processor can process examples in the wild - example from Atlassian") = {
    ...
    ClaimsProcessor
      .process(
        ...
        Duration.ofSeconds(current - 1386899131 + 60)
      )
      .isSuccess
  }
  ...
  property("processor can process examples in the wild - example from jwt.io") = {
    ...
    ClaimsProcessor
      .process(
        claims,
        Set("sub", "name", "admin"),
        Set(SubjectClaim("1234567890"), PublicClaim("name", "John Doe"), PrivateClaim("admin", true)),
        Duration.ZERO
      )
      .isSuccess
  }
