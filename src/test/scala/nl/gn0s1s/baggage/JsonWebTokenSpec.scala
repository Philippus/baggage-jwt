...
object JsonWebTokenSpec extends Properties("JsonWebToken") {
  property("can generate a jwt from a header and a key") = forAll {
  ...
  property("can generate a jwt from an algorithm and a key") = forAll {
    (alg: Algorithm, key: Key) =>
  ...
  property("jwt should contain at least two parts") = forAll {
    (alg: Algorithm, key: Key) =>
  ...
  property("decodes examples in the wild - example in rfc 7519") = {
    ...
    JsonWebToken(exampleFromRfc2).map(_.decode).get match {
      case Success((header, claims, _)) => header == JoseHeader(HS256, Some("JWT"), None) && claims == Set(IssuerClaim("joe"), ExpirationTimeClaim(1300819380), PrivateClaim("http://example.com/is_root", true))
  ...
  property("decodes examples in the wild - example in rfc 7519") = {
    val exampleFromRfc = """eyJhbGciOiJub25lIn0.eyJpc3MiOiJqb2UiLA0KICJleHAiOjEzMDA4MTkzODAsDQogImh0dHA6Ly9leGFtcGxlLmNvbS9pc19yb290Ijp0cnVlfQ."""
    JsonWebToken(exampleFromRfc).map(_.decode).get match {
      case Success((header, claims, _)) => header == JoseHeader(NoneAlgorithm, None, None) && claims == Set(IssuerClaim("joe"), ExpirationTimeClaim(1300819380), PrivateClaim("http://example.com/is_root", true))
      case Failure(_) => false
    }
  }
  ...
  property("decodes examples in the wild - example from atlassian") = {
    ...
    JsonWebToken(exampleFromAtlassian).map(_.decode).get match {
      case Success((header, claims, _)) => header == JoseHeader(HS256, Some("JWT"), None) && claims == Set(IssuerClaim("jira:15489595"), ExpirationTimeClaim(1386899131), PrivateClaim("qsh", "8063ff4ca1e41df7bc90c8ab6d0f6207d491cf6dad7c66ea797b4614b71922e9"), IssuedAtClaim(1386898951))
      case Failure(_) => false
    }
  }
  ...
  property("decodes examples in the wild - example from jwt.io") = {
    ...
    JsonWebToken(exampleFromJwtIo).map(_.decode).get match {
      case Success((header, claims, _)) => header == JoseHeader(HS256, Some("JWT"), None) && claims == Set(SubjectClaim("1234567890"), PublicClaim("name", "John Doe"), PrivateClaim("admin", true))
      case Failure(_) => false
    }
  }
  ...
  property("decodes examples in the wild - example from authentikat-jwt") = {
    ...
    JsonWebToken(exampleFromAuthentikat).map(_.decode).get match {
      case Success((header, claims, _)) => header == JoseHeader(HS256, Some("JWT"), None) && claims == Set(PrivateClaim("Hey", "foo"))
      case Failure(_) => false
    }
  }
}
